...
<head>
  ...
  <link
    href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500;600&family=IBM+Plex+Sans+Arabic:wght@400;500;600;700&display=swap"
    rel="stylesheet">
...
<body>
  ...
  <div id="app-shell" x-data="dashboard()" x-cloak :dir="ar ? 'rtl' : 'ltr'" @open-lock-modal.window="lockOpen = true"
    ...
    <div id="sidebar" :class="expanded ? '' : 'collapsed'">
      ...
      <div
        ...
        <div x-show="expanded" style="overflow:hidden">
          ...
          <div style="font-family:'JetBrains Mono',monospace;font-size:10.5px;color:#5C6678">{{ $version }}</div>
        </div>
      </div>
      ...
      <nav style="display:flex;flex-direction:column;gap:2px;padding:12px 10px;flex:1">
        ...
        <button class="nav-btn" :class="page==='settings'  ? 'active' : ''" @click="go('settings')">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"
            ...
            <path
              d="M19.4 15a1.6 1.6 0 0 0 .3 1.8l.1.1a2 2 0 1 1-2.8 2.8l-.1-.1a1.6 1.6 0 0 0-2.7 1.1V21a2 2 0 1 1-4 0v-.1A1.6 1.6 0 0 0 7.5 19.4l-.1.1a2 2 0 1 1-2.8-2.8l.1-.1A1.6 1.6 0 0 0 3 14.6H3a2 2 0 1 1 0-4h.1A1.6 1.6 0 0 0 4.6 7.5l-.1-.1a2 2 0 1 1 2.8-2.8l.1.1A1.6 1.6 0 0 0 10 3V3a2 2 0 1 1 4 0v.1a1.6 1.6 0 0 0 2.5 1.4l.1-.1a2 2 0 1 1 2.8 2.8l-.1.1a1.6 1.6 0 0 0 1.1 2.7H21a2 2 0 1 1 0 4h-.1a1.6 1.6 0 0 0-1.5 1z" />
          </svg>
      ...
      {{-- Footer --}}
      <div
        style="padding:12px 10px;border-top:1px solid #1B2130;display:flex;flex-direction:column;gap:8px;flex-shrink:0">
        <div x-show="expanded"
          style="display:flex;align-items:center;gap:8px;padding:9px 10px;border-radius:9px;background:#10141C;border:1px solid #1B2130;overflow:hidden">
          <span
            style="width:7px;height:7px;flex:none;border-radius:50%;background:#6EE7B7;animation:pulse 2.4s infinite"></span>
          <span
        ...
        <button @click="expanded = !expanded" class="nav-btn" style="justify-content:center">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
            stroke-linecap="round" style="flex:none;transition:.2s" :style="expanded ? '' : 'transform:rotate(180deg)'">
    ...
    <div id="main-col">
      ...
      <div id="top-bar">
        <div style="flex:1;min-width:0">
          <div
            style="font-size:16px;font-weight:600;letter-spacing:-.3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis"
            x-text="pageTitle"></div>
          ...
          <div style="position:relative;display:inline-block">
            <select onchange="window.location.href='{{ url('') }}/ai-translator/set-lang/' + this.value"
              ...
              @foreach($_allLangs as $code => [$name, $native])
                <option value="{{ $code }}" {{ $code === $_dashLang ? 'selected' : '' }}
                  style="background:#101420;color:#E6E9EF">
                  {{ $native }} ({{ strtoupper($code) }}){{ !in_array($code, $_preTranslated) ? ' ✦ AI' : '' }}
                </option>
              @endforeach
            </select>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#5C6678" stroke-width="2.5"
    ...
    <div class="modal-backdrop" x-show="restoreOpen" x-cloak @click.self="restoreOpen=false">
      <div
        style="width:100%;max-width:440px;border:1px solid #4A3A2A;border-radius:16px;background:#101420;padding:22px;box-shadow:0 30px 80px rgba(0,0,0,.6)">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px">
          <div
            style="width:38px;height:38px;flex:none;border-radius:11px;background:rgba(251,191,36,.1);border:1px solid rgba(251,191,36,.3);display:grid;place-items:center;color:#FBBF24">
        ...
        <div
          style="padding:12px 14px;border-radius:10px;background:#0D111A;border:1px solid #1B2130;margin-bottom:18px">
          <div style="font-family:'JetBrains Mono',monospace;font-size:12.5px;color:#E6E9EF;direction:ltr"
        ...
        <div style="display:flex;gap:10px;justify-content:flex-end">
          <button @click="restoreOpen=false"
            ...
            {{ $_trans['cancel'] ?? 'Cancel' }}></button>
          <button @click="confirmRestore()"
            style="padding:9px 16px;border-radius:10px;border:1px solid #FBBF24;background:#FBBF24;color:#2A1D02;font-size:13px;font-weight:600;cursor:pointer"
            {{ $_trans['restore_confirm'] ?? 'Yes, restore' }}></button>
        </div>
      </div>
    </div>
    ...
    <div class="modal-backdrop" x-show="lockOpen" x-cloak @click.self="lockOpen=false">
      <div class="modal-box">
        <div style="display:flex;align-items:center;padding:18px 20px;border-bottom:1px solid #1B2130">
          <div style="font-size:14px;font-weight:600;flex:1;color:#E6E9EF">
            {{ $_trans['lock_key_title'] ?? 'Lock a Key' }}
          </div>
          <button @click="lockOpen=false"
            style="width:28px;height:28px;display:grid;place-items:center;border-radius:8px;border:1px solid #1B2130;background:transparent;color:#5C6678;cursor:pointer;font-size:18px;line-height:1">×</button>
        </div>
        <div style="padding:18px 20px;display:flex;flex-direction:column;gap:14px">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
            <label>
              <span
                style="display:block;font-size:12px;color:#8B93A5;margin-bottom:7px">{{ $_trans['language'] ?? 'Language' }}</span>
              <select x-model="lockLang"
                ...
                @foreach(array_filter(config('ai-translator.languages', []), fn($l) => $l !== config('ai-translator.default_language', 'en')) as $l)
                  <option value="{{ $l }}">{{ strtoupper($l) }} —
                    {{ ['ar' => 'Arabic', 'fr' => 'French', 'es' => 'Spanish', 'de' => 'German', 'zh' => 'Chinese', 'ja' => 'Japanese', 'tr' => 'Turkish', 'ru' => 'Russian', 'pt' => 'Portuguese', 'ko' => 'Korean', 'nl' => 'Dutch', 'it' => 'Italian', 'pl' => 'Polish', 'hi' => 'Hindi', 'sv' => 'Swedish', 'vi' => 'Vietnamese', 'id' => 'Indonesian'][$l] ?? strtoupper($l) }}
                  </option>
                @endforeach
              </select>
            </label>
            <label>
              <span
                style="display:block;font-size:12px;color:#8B93A5;margin-bottom:7px">{{ $_trans['key'] ?? 'Key' }}</span>
              <input x-model="lockKey" placeholder="auth.login"
                style="width:100%;padding:10px 12px;border-radius:9px;border:1px solid #1B2130;background:#0D111A;color:#E6E9EF;font-family:'JetBrains Mono',monospace;font-size:12px;outline:none;direction:ltr">
            </label>
          </div>
          <label>
            <span
              style="display:block;font-size:12px;color:#8B93A5;margin-bottom:7px">{{ $_trans['reason_optional'] ?? 'Reason (optional)' }}</span>
        ...
        <div style="display:flex;gap:10px;justify-content:flex-end;padding:0 20px 18px">
          <button @click="lockOpen=false"
            style="padding:9px 16px;border-radius:10px;border:1px solid #2B3446;background:transparent;color:#8B93A5;font-size:13px;cursor:pointer">{{ $_trans['cancel'] ?? 'Cancel' }}</button>
      ...
      <div class="modal-box">
        <div style="display:flex;align-items:center;gap:12px;padding:18px 20px;border-bottom:1px solid #1B2130">
          ...
          <button @click="addOpen=false"
            style="width:28px;height:28px;display:grid;place-items:center;border-radius:8px;border:1px solid #1B2130;background:transparent;color:#5C6678;cursor:pointer;font-size:15px">×</button>
        </div>
        <div style="padding:16px 20px 20px">
          <input x-model="addQuery" placeholder="{{ $_trans['search_language'] ?? 'Search languages...' }}"
            style="width:100%;padding:10px 12px;border-radius:9px;border:1px solid #1B2130;background:#0D111A;color:#E6E9EF;font-size:13px;outline:none;margin-bottom:10px">
          <div style="display:flex;flex-direction:column;gap:6px;max-height:230px;overflow-y:auto">
            <template x-for="opt in filteredAddOptions" :key="opt.code">
              <button @click="pickLanguage(opt)"
                style="display:flex;align-items:center;gap:10px;width:100%;padding:10px 12px;border-radius:9px;border:1px solid #1B2130;background:#0D111A;color:#E6E9EF;cursor:pointer;text-align:start">
                <span
                  style="width:30px;height:22px;flex:none;border-radius:5px;background:#161C27;border:1px solid #2B3446;display:grid;place-items:center;font-family:'JetBrains Mono',monospace;font-size:10px;color:#8B93A5"
    ...
    <div
      ...
      <template x-for="toast in toasts" :key="toast.id">
        <div
          style="display:flex;align-items:center;gap:12px;min-width:260px;max-width:360px;padding:13px 16px;border-radius:12px;background:#141A26;border:1px solid #2B3446;box-shadow:0 16px 40px rgba(0,0,0,.5);animation:tin .22s ease">
          <span
            :style="'width:8px;height:8px;flex:none;border-radius:50%;background:' + (toast.kind==='err' ? '#F87171' : '#6EE7B7')"></span>
          <div>
  ...
  <script>
    ...
    function dashboard() {
      return {
        ...
        t: @json($_trans),
        async init() {
          this.startBackgroundPoller(); // ← add this line
          // ... rest of existing init code if any
        },
        ...
        get filteredAddOptions() {
          // Full ISO 639-1 language list — the AI translator can attempt any of these.
          const all = [
            { code: 'aa', name: 'Afar', native: 'Afaraf' },
            { code: 'ab', name: 'Abkhaz', native: 'аԥсуа бызшәа' },
            ...
            { code: 'ii', name: 'Sichuan Yi', native: 'ꆈꌠ꒿ Nuosuhxop' },
          ...
          // Hide the source language and anything already configured.
          const configuredCodes = this.configuredLangs.map(l => l.code);
          let available = all.filter(o => o.code !== this.sourceLang && !configuredCodes.includes(o.code));

          if (!this.addQuery) return available;
          const q = this.addQuery.toLowerCase();
          return available.filter(o => o.name.toLowerCase().includes(q) || o.native.toLowerCase().includes(q) || o.code.includes(q));
        },
