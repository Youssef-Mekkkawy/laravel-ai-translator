  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px">
    <div style="border:1px solid #1B2130;border-radius:14px;background:#101420;padding:20px">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">
        ...
        <button @click="doTranslate(false)" :disabled="!providerOk"
        ...
        <button @click="doTranslate(true)" :disabled="!providerOk"
...
function overviewPage() {
  return {
    ...
    async init() {
      ...
      // Resume progress bar if a translation was already running when page loaded
    ...
    async checkRunningOnLoad() {
      try {
        ...
        if (s.success && s.data?.running) {
          // A translation was already in progress — show the banner and start polling
          this.running     = true;
          this.progressPct = 30;
    ...
    async doTranslate(dryRun = false) {
      ...
      // Fake progress ticker — server doesn't stream progress
      this._tickTimer = setInterval(() => {
        if (this.progressPct < 85) this.progressPct += 3;
      }, 1500);

      try {
        // Snapshot the last_completed_at BEFORE queuing so we can detect
        // when OUR job finishes (vs a job that was already done)
        ...
        // Queue the job — server returns IMMEDIATELY, browser is free
        ...
        // Poll the status endpoint until OUR job completes
    ...
    startPolling(prevCompletedAt) {
      ...
      this._pollTimer = setInterval(async () => {
        try {
          ...
          // Detect completion: not running AND last_completed_at changed from before we started
