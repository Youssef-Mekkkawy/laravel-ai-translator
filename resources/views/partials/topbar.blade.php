{{-- Top bar: mobile menu button, page title, interface-language picker, theme toggle, avatar.
      ...
      <div class="lang-menu glass" :class="open ? 'is-open' : ''" :style="menuStyle" x-cloak role="menu">
        <template x-for="lang in allLangs" :key="lang.code">
          <button type="button" :class="lang.code === currentCode ? 'is-active' : ''" @click="selectLang(lang)">
            ...
            <span class="lang-menu-meta" x-text="' (' + lang.code.toUpperCase() + ')' + (lang.preTranslated ? '' : ' ✦ AI')"></span>
          </button>
        </template>
      </div>
    </div>

    {{-- Light / dark (wired up in dashboard.js) --}}
