...
<div x-data="lockedPage()" style="display:flex;flex-direction:column;gap:16px">
  ...
  <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
    <input x-model="search" placeholder="Search by key, language or value..."
  ...
  <div style="border:1px dashed #232B3B;border-radius:14px;background:#0E1219;padding:64px 24px;text-align:center">
    ...
    <div style="font-size:15px;font-weight:600;margin-bottom:6px">No locked keys</div>
    <div style="font-size:13px;color:#5C6678;margin-bottom:20px">Lock a translation to protect it from being overwritten by AI.</div>
    <button @click="$dispatch('open-lock-modal')"
    ...
    <table style="width:100%;border-collapse:collapse;min-width:600px">
      <thead>
        <tr style="background:#0E1219;border-bottom:1px solid #1B2130">
          <th style="padding:11px 16px;text-align:start;font-size:11px;text-transform:uppercase;letter-spacing:.8px;color:#5C6678;font-weight:600">Language</th>
          <th style="padding:11px 16px;text-align:start;font-size:11px;text-transform:uppercase;letter-spacing:.8px;color:#5C6678;font-weight:600">Key</th>
          <th style="padding:11px 16px;text-align:start;font-size:11px;text-transform:uppercase;letter-spacing:.8px;color:#5C6678;font-weight:600">Value</th>
          <th style="padding:11px 16px;text-align:start;font-size:11px;text-transform:uppercase;letter-spacing:.8px;color:#5C6678;font-weight:600">Locked by</th>
          <th style="padding:11px 16px;text-align:start;font-size:11px;text-transform:uppercase;letter-spacing:.8px;color:#5C6678;font-weight:600">Reason</th>
      ...
      <tbody>
        ...
        <tr style="border-top:1px solid #161C27"
          ...
          <td style="padding:13px 16px;font-size:13px;color:#8B93A5;max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
            {{ $lock['value'] ?? '—' }}
          </td>
          <td style="padding:13px 16px">
          ...
          <td style="padding:13px 16px;text-align:end">
            <button @click="unlock('{{ $lock['lang'] }}', '{{ $lock['key'] }}')"
              ...
              <span x-show="unlocking !== '{{ $lock['lang'] }}.{{ $lock['key'] }}'">Unlock</span>
            </button>
          </td>
        </tr>
        @endforeach
  ...
  <div style="font-size:12px;color:#5C6678">
    {{ count($locks) }} locked key(s)
  </div>
  @endif
  ...
  return {
    ...
    async unlock(lang, key) {
      ...
      try {
        ...
        this.msg   = r.message || (r.success ? 'Unlocked.' : 'Failed to unlock.');
