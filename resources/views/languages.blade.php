...
<div style="display:flex;flex-direction:column;gap:16px">
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
    <span style="font-size:13px;color:#5C6678">{{ count(array_filter($languages, fn($l) => $l['enabled'])) }} enabled of {{ count($languages) }}</span>
    <button @click="$store.dashboard ? $store.dashboard.addOpen = true : addOpen = true"
      onclick="document.querySelector('[x-data]').__x.$data.addOpen = true"
      ...
      + <span x-text="t.addLanguage">Add language</span>
    </button>
  </div>

  @if(count($languages) === 0)
  <div style="border:1px dashed #232B3B;border-radius:14px;background:#0E1219;padding:64px 24px;text-align:center">
    <div style="font-size:15px;font-weight:600;margin-bottom:8px">No languages configured</div>
    <div style="font-size:13px;color:#5C6678;margin-bottom:20px">Add languages in config/ai-translator.php</div>
  </div>
  @else
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px">
    ...
    <div style="border:1px solid #1B2130;border-radius:14px;background:#101420;padding:18px">
      ...
      <div style="display:flex;align-items:center;gap:12px;font-size:12.5px;color:#8B93A5">
        <span>{{ number_format($totalKeys) }} keys</span>
        <span style="color:#6EE7B7;font-weight:600">{{ $lang['pct'] }}%</span>
        @if($lang['missing'] > 0)
        <span style="color:#FBBF24">{{ number_format($lang['missing']) }} Missing</span>
        @else
        <span style="color:#6EE7B7">✓ Complete</span>
        @endif
      </div>
    </div>
    @endforeach
